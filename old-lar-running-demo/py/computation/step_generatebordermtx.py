...
def main(argv):
	...
	V = [(ind(x,y,z), [x,y,z]) for z in xrange(nz+1) for y in xrange(ny+1) for x in xrange(nx+1) ]

	log(3, ["V = " + str(V)])
	log(3, ["len(V) = " + str(len(V))])
	...
	CPV = [[x,y,z] for z in xrange(nz) for y in xrange(ny) for x in xrange(nx)]
	...
	for h in xrange(len(V)):
		...
		ix,iy,iz = x,y,z
		if (x < nx) and (y < ny): 
			FV.append([h,ind(ix+1,iy,iz),ind(ix,iy+1,iz),ind(ix+1,iy+1,iz)])
		if (x < nx) and (z < nz):
			FV.append([h,ind(ix+1,iy,iz),ind(ix,iy,iz+1),ind(ix+1,iy,iz+1)])
		if (y < ny) and (z < nz):
			FV.append([h,ind(ix,iy+1,iz),ind(ix,iy,iz+1),ind(ix,iy+1,iz+1)])
	...
	try:
		pass #computeBordo3(FV,CV,fileName)
	except:
